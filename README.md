...
Each row below is one commit. Where benchmarks exist they are filled in; rows still to come are marked.

| # | Change | Status | Val loss | Notes |
|---|--------|--------|----------|-------|
| 0 | Baseline GPT-2 (LayerNorm, learned PE, ReLU FFN, MHA) | ✅ | _TBD_ | Reference point |
...
The full benchmark table is regenerated by the script in `scripts/benchmark.py` (added in a later commit).
